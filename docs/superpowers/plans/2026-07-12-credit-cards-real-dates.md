...
### Task 4: Карты в `buildForecast` через явные даты + регрессия

**Files:**
- Modify: `src/finance.js:242-258` (блок карт в `buildForecast`)
- Test: `test/finance.test.js`

**Interfaces:**
- Consumes: `cardCycle`, `cardMinPayment`.
- Produces: событие карты (`kind: 'card'`) кладётся на `due` из `cardCycle(card, start)`, с `meta.graceDate = graceEnd`. Поведение payStrategy (`full` → весь долг, `minimum` → `cardMinPayment`) сохраняется.

- [ ] **Step 1: Написать падающий регресс-тест**

```js
test('buildForecast: карта с долгом попадает в события (регрессия)', () => {
  const state = {
  ...
  assert.ok(cardEvents.length >= 1, 'должно быть хотя бы одно событие карты')
...
Run: `node --test --test-name-pattern 'регрессия' 2>&1 | tail -20`
Expected: скорее всего PASS уже сейчас (блок карт использует `cardNextDue`, который делегирует в `cardCycle`). Если PASS — это подтверждает диагностику: событие формируется, просто `meta.graceDate` ещё нет. Всё равно продолжаем к шагу 3, чтобы добавить `graceDate` и убедиться в согласованности. Если FAIL — фиксируем на шаге 3.

- [ ] **Step 3: Обновить блок карт в `buildForecast`**

Заменить блок (строки ~242-258) на:
...
```bash
git add src/finance.js test/finance.test.js
git commit -m "buildForecast: карты через cardCycle, добавить graceDate в событие"
```
